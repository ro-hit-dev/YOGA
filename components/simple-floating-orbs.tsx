...
interface SimpleFloatingOrbsProps {
  orbCount?: number
  colors?: string[]
  interactive?: boolean
  className?: string
...
export default function SimpleFloatingOrbs({
  orbCount = 6,
  colors = ["#a4ac86", "#c2c5aa", "#b6ad90", "#936639", "#7f4f24"],
  interactive = true,
  className = "",
}: SimpleFloatingOrbsProps) {
  const containerRef = useRef<HTMLDivElement>(null)
  const [orbs, setOrbs] = useState<SimpleOrb[]>([])
  ...
  useEffect(() => {
    ...
    const newOrbs: SimpleOrb[] = Array.from({ length: orbCount }, (_, i) => {
  ...
  return (
    <div ref={containerRef} className={`absolute inset-0 pointer-events-none overflow-hidden ${className}`}>
      {orbs.map((orb, index) => {
        const mouseDistance = Math.sqrt((currentMouseX - orb.x) ** 2 + (currentMouseY - orb.y) ** 2)
        const scale = Math.max(1, Math.min(1.5, 1 + ((200 - Math.min(mouseDistance, 200)) / 200) * 0.5))
        const opacity = Math.max(0.3, Math.min(0.8, 0.3 + ((200 - Math.min(mouseDistance, 200)) / 200) * 0.5))
